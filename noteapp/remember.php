<?php
//if the user is not logged in and rememverme cookie exists
   ...
   if(!isset($_SESSION['user_id']) && !empty($_COOKIE['rememberme'])){
    ...
    //extract $authenticators 1 and 2 from the cookie
       ...
       $sql= "SELECT * FROM rememberme WHERE authentificator1='$authentificator1'";
        ...
        if(!$result){
            echo '<div class="alert alaert-danger" >There was an error running the query.</div>';exit;
        }
        $count = mysqli_num_rows($result);

if($count!==1){
    echo '<div class="alert alaert-danger" >Remember me process  failed<div>';
    exit; 
} 
    $row = mysqli_fetch_array($result, MYSQLI_ASSOC);
       //IF authentificator2 does not match
       if(!hash_equals($row['f2authentificator2'],$f2authentificator2)){
            echo '<div class="alert alaert-danger" >hash equals returned false!<div>';
       }else{
            //genetate new authentificators
           //store them in cookie anad rememberme table
